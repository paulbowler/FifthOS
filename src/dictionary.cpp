...
void initDictionary()
{
    ...
    HEADER(8, "GFX.INIT");
    int GFXINI = CODE(4, as_gfx_init, as_next, 0, 0);
    HEADER(8, "SCREEN.W");
    int SCRNW = CODE(4, as_screen_w, as_next, 0, 0);
    HEADER(8, "SCREEN.H");
    int SCRNH = CODE(4, as_screen_h, as_next, 0, 0);
    HEADER(3, "CLS");
    int CLS = CODE(4, as_cls, as_next, 0, 0);
    HEADER(5, "PIXEL");
    int PIXEL = CODE(4, as_pixel, as_next, 0, 0);
    HEADER(4, "RECT");
    int GRECT = CODE(4, as_rect, as_next, 0, 0);
    HEADER(4, "TEXT");
    int TEXT = CODE(4, as_text, as_next, 0, 0);
    HEADER(6, "TEXT-W");
    ...
    int TUPDAT = CODE(4, as_touch_update, as_next, 0, 0);
    HEADER(10, "TOUCH.TYPE");
    int TTYPE = CODE(4, as_touch_type, as_next, 0, 0);
    HEADER(7, "TOUCH.X");
    int TTOUCHX = CODE(4, as_touch_x, as_next, 0, 0);
    HEADER(7, "TOUCH.Y");
    int TTOUCHY = CODE(4, as_touch_y, as_next, 0, 0);
    HEADER(8, "TOUCH.DX");
    int TTOUCHDX = CODE(4, as_touch_dx, as_next, 0, 0);
    HEADER(8, "TOUCH.DY");
    int TTOUCHDY = CODE(4, as_touch_dy, as_next, 0, 0);
    HEADER(10, "TOUCH.TICK");
    int TTOUCHT = CODE(4, as_touch_tick, as_next, 0, 0);
    HEADER(12, "TOUCH.TARGET");
    int TTOUCHG = CODE(4, as_touch_target, as_next, 0, 0);
    HEADER(12, "SWIPE.DETECT");
    int SWIPED = CODE(4, as_swipe_detect, as_next, 0, 0);
    HEADER(9, "STYLE.NEW");
    ...
    HEADER(8, "NODE.ADD");
    int NODEADD = CODE(4, as_node_add, as_next, 0, 0);
    HEADER(11, "NODE.REMOVE");
    int NODEREM = CODE(4, as_node_remove, as_next, 0, 0);
    HEADER(10, "NODE.DIRTY");
    int NODEDIR = CODE(4, as_node_dirty, as_next, 0, 0);
    HEADER(9, "NODE.DRAW");
    int NODEDRW = CODE(4, as_node_draw, as_next, 0, 0);
    HEADER(14, "NODE.DRAW-TREE");
    int NODETREE = CODE(4, as_node_draw_tree, as_next, 0, 0);
    HEADER(8, "NODE.HIT");
    int NODEHIT = CODE(4, as_node_hit, as_next, 0, 0);
    HEADER(14, "NODE.SCREEN-XY");
    int NODESXY = CODE(4, as_node_screen_xy, as_next, 0, 0);
    HEADER(15, "NODE.SET-BOUNDS");
    int NODESET = CODE(4, as_node_set_bounds, as_next, 0, 0);
    HEADER(13, "NODE.VISIBLE?");
    int NODEVIS = CODE(4, as_node_visible, as_next, 0, 0);
    HEADER(15, "NODE.TOUCHABLE?");
    int NODETOU = CODE(4, as_node_touchable, as_next, 0, 0);
    HEADER(11, "NODE.FIELD@");
    int NODEAT = CODE(4, as_node_field_get, as_next, 0, 0);
    HEADER(11, "NODE.FIELD!");
    int NODEST = CODE(4, as_node_field_set, as_next, 0, 0);
    HEADER(14, "EVENT.DISPATCH");
    int EVDISP = CODE(4, as_event_dispatch, as_next, 0, 0);
    HEADER(12, "EVENT.BUBBLE");
    int EVBUB = CODE(4, as_event_bubble, as_next, 0, 0);
    HEADER(7, "APP.NEW");
    int APPNEW = CODE(4, as_app_new, as_next, 0, 0);
    HEADER(14, "APP.ADD-SCREEN");
    int APPADD = CODE(4, as_app_add_screen, as_next, 0, 0);
    HEADER(8, "APP.NEXT");
    int APPNEXT = CODE(4, as_app_next, as_next, 0, 0);
    HEADER(8, "APP.PREV");
    int APPPREV = CODE(4, as_app_prev, as_next, 0, 0);
    HEADER(8, "APP.SHOW");
    int APPSHOW = CODE(4, as_app_show, as_next, 0, 0);
    HEADER(10, "APP.ACTIVE");
    int APPACT = CODE(4, as_app_active, as_next, 0, 0);
    HEADER(8, "APP.DRAW");
    int APPDRAW = CODE(4, as_app_draw, as_next, 0, 0);
    HEADER(7, "W.PANEL");
    int WPANEL = CODE(4, as_w_panel, as_next, 0, 0);
    HEADER(7, "W.LABEL");
    int WLABEL = CODE(4, as_w_label, as_next, 0, 0);
    HEADER(8, "W.BUTTON");
    int WBUTTON = CODE(4, as_w_button, as_next, 0, 0);
    HEADER(8, "W.STATUS");
    int WSTATUS = CODE(4, as_w_status, as_next, 0, 0);
    HEADER(7, "W.VALUE");
    int WVALUE = CODE(4, as_w_value, as_next, 0, 0);
    HEADER(7, "W.GAUGE");
    int WGAUGE = CODE(4, as_w_gauge, as_next, 0, 0);
    HEADER(7, "W.CHART");
    int WCHART = CODE(4, as_w_chart, as_next, 0, 0);
    HEADER(7, "W.ALARM");
    int WALARM = CODE(4, as_w_alarm, as_next, 0, 0);
    HEADER(8, "W.ICON16");
    int WICON16 = CODE(4, as_w_icon16, as_next, 0, 0);
    HEADER(11, "WIFI.STATUS");
    int WIFISTATUS = CODE(4, as_wifi_status, as_next, 0, 0);
    HEADER(9, "WIFI.SCAN");
    int WIFISCAN = CODE(4, as_wifi_scan, as_next, 0, 0);
    HEADER(12, "WIFI.CONNECT");
    int WIFICONNECT = CODE(4, as_wifi_connect, as_next, 0, 0);
    HEADER(8, "WIFI.WPS");
    int WIFIWPS = CODE(4, as_wifi_wps, as_next, 0, 0);
    HEADER(11, "WIFI.FORGET");
    int WIFIFORGET = CODE(4, as_wifi_forget, as_next, 0, 0);
    HEADER(15, "WIFI.CONNECTED?");
    int WIFICONNECTED = CODE(4, as_wifi_connected, as_next, 0, 0);
    HEADER(8, "WIFI.AP?");
    int WIFIAP = CODE(4, as_wifi_ap, as_next, 0, 0);
    HEADER(11, "WIFI.SAVED?");
    int WIFISAVED = CODE(4, as_wifi_saved, as_next, 0, 0);
    HEADER(6, "MILLIS");
    int MILLIS = CODE(4, as_millis_now, as_next, 0, 0);
    HEADER(10, "TIME.SYNC?");
    int TIMESYNC = CODE(4, as_time_sync, as_next, 0, 0);
    HEADER(9, "TIME.UNIX");
    int TIMEUNIX = CODE(4, as_time_unix, as_next, 0, 0);
    HEADER(8, "TIME.HMS");
    int TIMEHMS = CODE(4, as_time_hms, as_next, 0, 0);
    HEADER(8, "TIME.YMD");
    ...
    int TASKEVERY = CODE(4, as_task_every, as_next, 0, 0);
    HEADER(9, "TASK.ONCE");
    int TASKONCE = CODE(4, as_task_once, as_next, 0, 0);
    HEADER(10, "TASK.START");
    int TASKSTART = CODE(4, as_task_start, as_next, 0, 0);
    HEADER(9, "TASK.STOP");
    int TASKSTOP = CODE(4, as_task_stop, as_next, 0, 0);
    HEADER(12, "TASK.ACTIVE?");
    int TASKACTIVE = CODE(4, as_task_active, as_next, 0, 0);
    HEADER(12, "LOCALS.ENTER");
    ...
    HEADER(4, "LINE");
    int GFXLIN = CODE(4, as_gfx_line, as_next, 0, 0);
    HEADER(9, "FILL-RECT");
    int FILLRECT = CODE(4, as_fill_rect, as_next, 0, 0);
    HEADER(10, "TOUCH.DOWN");
    int TDOWN = CODE(8, as_docon, as_next, 0, 0, EVENT_TOUCH_DOWN, 0, 0, 0);
    HEADER(10, "TOUCH.MOVE");
    int TMOVE = CODE(8, as_docon, as_next, 0, 0, EVENT_TOUCH_MOVE, 0, 0, 0);
    HEADER(8, "TOUCH.UP");
    int TUP = CODE(8, as_docon, as_next, 0, 0, EVENT_TOUCH_UP, 0, 0, 0);
    HEADER(9, "TOUCH.TAP");
    int TTAP = CODE(8, as_docon, as_next, 0, 0, EVENT_TOUCH_TAP, 0, 0, 0);
    HEADER(16, "TOUCH.SWIPE.LEFT");
    int TSWIPL = CODE(8, as_docon, as_next, 0, 0, EVENT_TOUCH_SWIPE_LEFT, 0, 0, 0);
    HEADER(17, "TOUCH.SWIPE.RIGHT");
    int TSWIPR = CODE(8, as_docon, as_next, 0, 0, EVENT_TOUCH_SWIPE_RIGHT, 0, 0, 0);
    HEADER(5, "TIMER");
    int TIMER = CODE(8, as_docon, as_next, 0, 0, EVENT_TIMER, 0, 0, 0);
    HEADER(4, "DRAW");
    int DRAW = CODE(8, as_docon, as_next, 0, 0, EVENT_DRAW, 0, 0, 0);
    ...
    IF(2, LITER, EXITT);
    THEN(1, ERRORR);
    forthWordScomp = SCOMP;
    HEADER(5, "OVERT");
    int OVERT = COLON(5, LAST, AT, CNTXT, STORE, EXITT);
    HEADER(1, "]");
    int RBRAC = COLON(5, DOLIT, SCOMPLOC, TEVAL, STORE, EXITT);
    HEADER(1, ":");
    int COLN = COLON(8, LOCEND, TOKEN, SNAME, RBRAC, DOLIT, 0x6, COMMA, EXITT);
    HEADER(IMEDD + 1, ";");
    int SEMIS = COLON(7, DOLIT, EXITT, COMMA, LOCEND, LBRAC, OVERT, EXITT);
    HEADER(3, "dm+");
